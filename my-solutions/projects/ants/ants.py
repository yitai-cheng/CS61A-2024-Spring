...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            assert self.can_contain(place.ant) is True or place.ant.can_contain(
                self) is True, 'Too many ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not cur_place.is_hive:
            if len(cur_place.bees) > 0 and self.lower_bound <= cur_place_index <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for i in range(len(bee_list)):
            bee_list[i].reduce_health(total_damage)
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        bee_list = list(self.place.bees)
        for i in range(len(bee_list)):
            bee_list[i].reduce_health(self.damage)
